...
for file in files:
    ...
    t_end = len(target_metro)
    f1_list = []
    f2_list = []
    f3_list = []
    f4_list = []
    for i in range(t_end + 2, t_end + 2 + len(metro_pre)):
    ...
    # process roi data
    pairs = np.unique(np.array(roi_sample_use.set_index(['src_place_id', 'dest_place_id', 'timestamp_iso']).index))
    rows = []
    # processing metro data first
    for i in pairs:
        df = roi_sample_use[(roi_sample_use['src_place_id'] == i[0])
                              & (roi_sample_use['dest_place_id'] == i[1]) &
                              (roi_sample_use['timestamp_iso'] == i[2])]
        row_current = defdict.copy()
        ...
        rows.append(row_current)
    roi_pre = pd.DataFrame(rows)
    roi_pre['source_country'] = 'India'
    s_n = target_roi['S. No.'].values[-1]
    num_list = []
    for i in range(len(metro_pre)):
        num_list.append(i + s_n + 1)
    
    roi_pre['S. No.'] = num_list

    # truncating at the top to keep under 1 million rows
    day_1_roi = target_roi.iloc[0]['Date']
    target_roi = target_roi[target_roi['Date'] > day_1_roi]
    roi_pre.replace(True, 'Y', inplace=True)
    roi_pre.replace(False, 'N', inplace=True)

       ## read and add formulae here 
    # read from text file. in order of, Uber_auto, Uber_go, Uber_premium, Uber_moto

    t_end = len(target_roi)
    f1_list = []
    f2_list = []
    f3_list = []
    f4_list = []
    for i in range(t_end + 2, t_end + 2 + len(roi_pre)):
        f1 = f'=IF(BG{i}<>0,IF(D{i}="Chennai",BG{i}-MIN(BG{i}*5%,50),IF(D{i}="Bangalore",BG{i}-MIN(BG{i}*6%,50),IF(D{i}="Mumbai",BG{i}-MIN(BG{i}*1%,50),IF(D{i}="Hyderabad",BG{i}-MIN(BG{i}*4%,40),BG{i})))),BG{i})'
        f2 = f'=IF(BL{i}<>0,IF(D{i}="Chennai",BL{i}-MIN(BL{i}*5%,50),IF(D{i}="Bangalore",BL{i}-MIN(BL{i}*6%,50),IF(D{i}="Mumbai",BL{i}-MIN(BL{i}*1%,50),IF(D{i}="Hyderabad",BL{i}-MIN(BL{i}*4%,40),BL{i})))),BL{i})'
        f3 = f'=IF(BB{i}<>0,IF(D{i}="Hyderabad",BB{i}-MIN(BB{i}*7%,10),IF(D{i}="Chennai",BB{i}-MIN(BB{i}*7%,10),IF(D{i}="Delhi NCR",BB{i}-MIN(BB{i}*7%,10),BB{i}))),BB{i})'
        f4 = f'=IF(AND(AW{i}<>0,D{i}<>"Kolkata"),IF(D{i}="Bangalore",(AW{i}-25),IF(D{i}="Delhi NCR",(AW{i}-25),IF(D{i}="Mumbai",(AW{i}-25),IF(D{i}="Hyderabad",(AW{i}-25),IF(D{i}="Chennai",(AW{i}-25),IF(D{i}="Pune",(AW{i}-25),IF(D{i}="Ahmedabad",(AW{i}-29)))))))),"")'
    ...
    roi_pre['Uber_Go_discounted_fare'] = f2_list
    roi_pre['Uber_Premier_discounted_fare'] = f3_list
    roi_pre['Uber_moto_discounted_fare'] = f4_list
    roi_pre['Uber_auto_discounted_fare'] = f1_list

    output_roi = pd.concat([target_metro, metro_pre])

    # save metro output
    output_metro.to_csv('/home/siddharth/uber_auto/output/data_roi.csv', index=False)
...
u_path = '/home/siddharth/uber_auto/output/'

f1 = open('output_metro.csv', 'rb')
f2 = open('output_roi.csv', 'rb')
file_size = os.path.getsize('output_metro.csv')
dest_path = '/output_metro.csv'

CHUNK_SIZE = 4 * 1024 * 1024

if file_size <= CHUNK_SIZE:

    print (dbx.files_upload(f1, dest_path))
...
file_size = os.path.getsize('output_roi.csv')
dest_path = '/output_roi.csv'

CHUNK_SIZE = 4 * 1024 * 1024

if file_size <= CHUNK_SIZE:

    print (dbx.files_upload(f2, dest_path))
